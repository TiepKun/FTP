...
inline bool recv_line(int sockfd, string &line) {
    line.clear();
    char c;
    while (true) {
...
// Gửi đủ len bytes
inline bool send_all(int sockfd, const void *buf, size_t len) {
...
// Gửi 1 dòng text có '\n'
inline bool send_line(int sockfd, const string &line) {
    string tmp = line;
...
inline vector<string> split_tokens(const string &s) {
